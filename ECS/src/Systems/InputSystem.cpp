...
void InputSystem::update(EntityManager& entityManager, gba_os::chrono::gba_microseconds deltaTime) {
    scanKeys();
    ...
    using Clock = gba_os::chrono::GbaClock;
    ...
    for (auto& pair : events[ButtonEventType::ButtonHeld300Ms]) {
        if (keys_held & pair.first) {
            ...
            if (now - timestamp > gba_os::chrono::gba_milliseconds(300)) {
                pair.second();
                timestamp = now;
            }
        }
    ...
    for (auto& pair : events[ButtonEventType::ButtonDoubleTap]) {
        if (keys_down & pair.first) {
            ...
            if (tapped && now - timestamp < gba_os::chrono::gba_milliseconds(300)) {
                pair.second();
                tapped = false;
            } else {
                tapped = true;
